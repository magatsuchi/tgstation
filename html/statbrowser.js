...
let decoder = decodeURIComponent || unescape;
...
let tickets = [];
let interviewManager = { status: "", interviews: [] };
let sdql2 = [];
let permanent_tabs = []; // tabs that won't be cleared by wipes
let turfcontents = [];
let turfname = "";
let imageRetryDelay = 500;
let imageRetryLimit = 50;
let menu = document.getElementById("menu");
let under_menu = document.getElementById("under_menu");
let statcontentdiv = document.getElementById("statcontent");
let storedimages = [];
...
function remove_verb(v) {
	let verb_to_remove = v; // to_remove = [verb:category, verb:name]
	for (let i = verbs.length - 1; i >= 0; i--) {
		let part_to_remove = verbs[i];
...
function tab_change(tab) {
	...
	if (document.getElementById(current_tab)) {
		document.getElementById(current_tab).className = "button";
	} // disable active on last button
	current_tab = tab;
	set_byond_tab(tab);
	if (document.getElementById(tab)) {
		document.getElementById(tab).className = "button active";
...
function draw_debug() {
	...
	let wipeUpdateVerbsTabs = document.createElement("div");
	...
	for (var i = 0; i < verb_tabs.length; i++) {
		...
		if (verb_tabs[i].lastIndexOf(".") != -1) {
			let splitName = verb_tabs[i].split(".");
			if (split_admin_tabs && splitName[0] === "Admin") {
				part = splitName[1];
			} else {
				continue;
...
function iconError(e) {
	...
	setTimeout(() => {
		let node = e.target;
		let current_attempts = Number(node.getAttribute("data-attempts")) || 0;
...
function draw_interviews() {
	...
	let manLink = document.createElement("a");
	...
	let statsDiv = document.createElement("table");
	statsDiv.className = "interview_panel_stats";
	for (let key in interviewManager.status) {
		let d = document.createElement("div");
		var tr = document.createElement("tr");
		let stat_name = document.createElement("td");
		let stat_text = document.createElement("td");
	...
	for (let i = 0; i < interviewManager.interviews.length; i++) {
		let part = interviewManager.interviews[i];
		var tr = document.createElement("tr");
...
function draw_verbs(cat) {
	...
	let additions = {}; // additional sub-categories to be rendered
	...
	let content = document.getElementById("statcontent");
...
window.set_theme = function (which) {
	if (which == "light") {
...
function add_verb_list(payload) {
	let to_add = payload; // list of a list with category and verb inside it
